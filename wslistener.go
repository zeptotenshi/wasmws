...
//ServeHTTPWithOpts is an exact copy of ServeHTTP but allows AcceptOptions
// added for server testing (i.e. websocket.AcceptOptions{ InsecureSkipVerify: true })
func (wsl *WebSockListener) ServeHTTPWithOpts(wtr http.ResponseWriter, req *http.Request, opts *websocket.AcceptOptions) {
	select {
	case <-wsl.ctx.Done():
		http.Error(wtr, "503: Service is shutdown", http.StatusServiceUnavailable)
		log.Printf("WebSockListener: WARN: A websocket listener's HTTP Accept was called when shutdown!")
	...
	ws, err := websocket.Accept(wtr, req, opts)
	if err != nil {
		log.Printf("WebSockListener: ERROR: Could not accept websocket from %q; Details: %s", req.RemoteAddr, err)
	}
